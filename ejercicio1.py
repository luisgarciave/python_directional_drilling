...
BUR = 1.5; # [°/30 m]
AZI = 219.2 # [°]
etha = 219.2 -180 #angulo ente EWo y DHo
#%%
L = 30; # [m]
#%%
R = (180/(BUR*np.pi))*L; # [m]
EWo = DHo * np.cos(etha*np.pi/180); #[m]
NSo = DHo * np.sin(etha*np.pi/180); #[m]
...
if R < DHo:
    ...
    OD = ((CD)**2 + (OC)**2)**0.5; #[ft]
    BC = ((OD)**2 - (R)**2)**0.5; #[ft]
    gamma = np.arccos( (R/OC))*(180/np.pi); #deg, arctan(co/hip)
    theta = 90 - gamma;
elif R> DHo:
    OD = TVDo - KOP; #[ft]
    CD =R - DHo; #[ft]
    OC = ((CD)**2 + (OD)**2)**0.5; #[ft]
    BC = ((OC)**2 - (R)**2)**0.5; #[ft]
    gamma = np.arctan( (CD/OD))*(180/np.pi); #deg, arctan(co/hip)
...
fig = go.Figure(data=[go.Table(header=dict(values=['   ', 'TVD', 'DH', 'MD']),
                 cells=dict(values=[['LOC', 'KOP', 'EOB', 'OBJ'], [0.0, KOP, EOB_TVD, TVDo],[0.0, 0.0, EOB_DH, DHo],[0.0, KOP, EOB_MD, MDo]]))
                     ])
fig.show()
